...
class Trainer(object):
    ...
    def __init__(self, B, T, g_E, g_H, d_E, d_H, d_dropout, path_pos, path_neg, g_lr=1e-3, d_lr=1e-3, n_sample=16, generate_samples=10000, init_eps=0.1):
        ...
        self.g_data = GeneratorPretrainingGenerator(self.path_pos, B=B, T=T, min_count=1)
        # next方法产生 pos数据和neg数据
        self.d_data = DiscriminatorGenerator(path_pos=self.path_pos, path_neg=self.path_neg, B=self.B, shuffle=True)
    ...
    def train(self, steps=10, g_steps=1, d_steps=1, d_epochs=5,
